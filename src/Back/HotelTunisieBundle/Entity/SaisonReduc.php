 ...
 * SaisonReduc
 *
 * @ORM\Table(name="ost_sht_saison_reduc")
 * @ORM\Entity
 */
class SaisonReduc
{
     ...
     * Refuction 3 eme lit
     */
    public function getReduc3LitAchat($arrangement = null)
    {
        if ($this->reduc3LitPour)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->reduc3Lit / 100) * -1;
        else
            $reduc = $this->reduc3Lit * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc3LitVente($arrangement = null)
    {
        if ($this->reduc3LitPour)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->reduc3Lit / 100) * -1;
        else
            $reduc = $this->reduc3Lit * -1;

        if ($this->reduc3LitMargePour)
            $marge = abs($reduc) * $this->reduc3LitMarge / 100;
        else
            $marge = $this->reduc3LitMarge;
     ...
     * Reduction 4 eme lit
     */
    public function getReduc4LitAchat($arrangement = null)
    {
        if ($this->reduc4LitPour)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->reduc4Lit / 100) * -1;
        else
            $reduc = $this->reduc4Lit * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc4LitVente($arrangement = null)
    {
        if ($this->reduc4LitPour)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->reduc4Lit / 100) * -1;
        else
            $reduc = $this->reduc4Lit * -1;

        if ($this->reduc4LitMargePour)
            $marge = abs($reduc) * $this->reduc4LitMarge / 100;
        else
            $marge = $this->reduc4LitMarge;

        return number_format($reduc + $marge, 3, '.', '');
    }

    /**
     * 1er enfant avec 1 adulte dans une même chambre
     */
    public function getReducEnfant1Adulte1Age1Achat($arrangement = null)
    {
        if ($this->pour1Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val1Enfant1Adulte1 / 100) * -1;
        else
            $reduc = $this->val1Enfant1Adulte1 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReducEnfant1Adulte1Age1Vente($arrangement = null)
    {
        if ($this->pour1Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val1Enfant1Adulte1 / 100) * -1;
        else
            $reduc = $this->val1Enfant1Adulte1 * -1;
        if ($this->margepour1Enfant1Adulte)
            $marge = abs($reduc) * $this->marge1Enfant1Adulte / 100;
        else
            $marge = $this->marge1Enfant1Adulte;
        return number_format($reduc + $marge, 3, '.', '');
    }

    public function getReducEnfant1Adulte1Age2Achat($arrangement = null)
    {
        if ($this->pour1Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val1Enfant1Adulte2 / 100) * -1;
        else
            $reduc = $this->val1Enfant1Adulte2 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReducEnfant1Adulte1Age2Vente($arrangement = null)
    {
        if ($this->pour1Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val1Enfant1Adulte2 / 100) * -1;
        else
            $reduc = $this->val1Enfant1Adulte2 * -1;
        if ($this->margepour1Enfant1Adulte)
            $marge = abs($reduc) * $this->marge1Enfant1Adulte / 100;
    ...
    /**
     * 1er enfant avec 2 adultes ou plus dans une même chambre
     */
    public function getReducEnfant1Adulte2Age1Achat($arrangement = null)
    {
        if ($this->pour1Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val1Enfant2Adulte1 / 100) * -1;
        else
            $reduc = $this->val1Enfant2Adulte1 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReducEnfant1Adulte2Age1Vente($arrangement = null)
    {
        if ($this->pour1Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val1Enfant2Adulte1 / 100) * -1;
        else
            $reduc = $this->val1Enfant2Adulte1 * -1;

        if ($this->margepour1Enfant2Adulte)
            $marge = abs($reduc) * $this->marge1Enfant2Adulte / 100;
        else
            $marge = $this->marge1Enfant2Adulte;
        return number_format($reduc + $marge, 3, '.', '');
    }

    public function getReducEnfant1Adulte2Age2Achat($arrangement = null)
    {
        if ($this->pour1Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val1Enfant2Adulte2 / 100) * -1;
        else
            $reduc = $this->val1Enfant2Adulte2 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReducEnfant1Adulte2Age2Vente($arrangement = null)
    {
        if ($this->pour1Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val1Enfant2Adulte2 / 100) * -1;
        else
            $reduc = $this->val1Enfant2Adulte2 * -1;
    ...
    /**
     * 1er enfant dans une chambre séparée
     */
    public function getReduc1EnfantSepare1Age1Achat($arrangement = null)
    {
        if ($this->pour1EnfantSepare)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val1EnfantSepare1 / 100) * -1;
        else
            $reduc = $this->val1EnfantSepare1 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc1EnfantSepare1Age1Vente($arrangement = null)
    {
        if ($this->pour1EnfantSepare)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val1EnfantSepare1 / 100) * -1;
        else
            $reduc = $this->val1EnfantSepare1 * -1;
        if ($this->margepour1EnfantSepare)
            $marge = abs($reduc) * $this->marge1EnfantSepare / 100;
        else
            $marge = $this->marge1EnfantSepare;
        return number_format($reduc + $marge, 3, '.', '');
    }

    public function getReduc1EnfantSepare1Age2Achat($arrangement = null)
    {
        if ($this->pour1EnfantSepare)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val1EnfantSepare2 / 100) * -1;
        else
            $reduc = $this->val1EnfantSepare2 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc1EnfantSepare1Age2Vente($arrangement = null)
    {
        if ($this->pour1EnfantSepare)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val1EnfantSepare2 / 100) * -1;
        else
            $reduc = $this->val1EnfantSepare2 * -1;
    ...
    /**
     * 2eme enfant ou plus avec 1 adulte dans une même chambre
     */
    public function getReduc2Enfant1Adulte1Age1Achat($arrangement = null)
    {
        if ($this->pour2Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val2Enfant1Adulte1 / 100) * -1;
        else
            $reduc = $this->val2Enfant1Adulte1 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc2Enfant1Adulte1Age1Vente($arrangement = null)
    {
        if ($this->pour2Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val2Enfant1Adulte1 / 100) * -1;
        else
            $reduc = $this->val2Enfant1Adulte1 * -1;
        if ($this->margepour2Enfant1Adulte)
            $marge = abs($reduc) * $this->marge2Enfant1Adulte / 100;
        else
            $marge = $this->marge2Enfant1Adulte;
        return number_format($reduc + $marge, 3, '.', '');
    }

    public function getReduc2Enfant1Adulte1Age2Achat($arrangement = null)
    {
        if ($this->pour2Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val2Enfant1Adulte2 / 100) * -1;
        else
            $reduc = $this->val2Enfant1Adulte2 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc2Enfant1Adulte1Age2Vente($arrangement = null)
    {
        if ($this->pour2Enfant1Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val2Enfant1Adulte2 / 100) * -1;
        else
            $reduc = $this->val2Enfant1Adulte2 * -1;
        if ($this->margepour2Enfant1Adulte)
            $marge = abs($reduc) * $this->marge2Enfant1Adulte / 100;
    ...
    /**
     * 2eme enfant ou plus avec 2 adultes ou plus dans une même chambre
     */
    public function getReduc2Enfant2Adulte1Age1Achat($arrangement = null)
    {
        if ($this->pour2Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val2Enfant2Adulte1 / 100) * -1;
        else
            $reduc = $this->val2Enfant2Adulte1 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc2Enfant2Adulte1Age1Vente($arrangement = null)
    {
        if ($this->pour2Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val2Enfant2Adulte1 / 100) * -1;
        else
            $reduc = $this->val2Enfant2Adulte1 * -1;
        if ($this->margepour2Enfant2Adulte)
            $marge = abs($reduc) * $this->marge2Enfant2Adulte / 100;
        else
            $marge = $this->marge2Enfant2Adulte;
        return number_format($reduc + $marge, 3, '.', '');
    }

    public function getReduc2Enfant2Adulte1Age2Achat($arrangement = null)
    {
        if ($this->pour2Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val2Enfant2Adulte2 / 100) * -1;
        else
            $reduc = $this->val2Enfant2Adulte2 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc2Enfant2Adulte1Age2Vente($arrangement = null)
    {
        if ($this->pour2Enfant2Adulte)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val2Enfant2Adulte2 / 100) * -1;
        else
            $reduc = $this->val2Enfant2Adulte2 * -1;
        if ($this->margepour2Enfant2Adulte)
            $marge = abs($reduc) * $this->marge2Enfant2Adulte / 100;
    ...
    /**
     * 2eme enfant ou plus dans une chambre séparée
     */
    public function getReduc2EnfantOuPlusSepare1Age1Achat($arrangement = null)
    {
        if ($this->pour2EnfantOuPlusSepare)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val2EnfantOuPlusSepare1 / 100) * -1;
        else
            $reduc = $this->val2EnfantOuPlusSepare1 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc2EnfantOuPlusSepare1Age1Vente($arrangement = null)
    {
        if ($this->pour2EnfantOuPlusSepare)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val2EnfantOuPlusSepare1 / 100) * -1;
        else
            $reduc = $this->val2EnfantOuPlusSepare1 * -1;
        if ($this->margepour2EnfantOuPlusSepare)
            $marge = abs($reduc) * $this->marge2EnfantOuPlusSepare / 100;
        else
            $marge = $this->marge2EnfantOuPlusSepare;
        return number_format($reduc + $marge, 3, '.', '');
    }

    public function getReduc2EnfantOuPlusSepare1Age2Achat($arrangement = null)
    {
        if ($this->pour2EnfantOuPlusSepare)
            $reduc = ($this->getSaison()->prixBaseAchat($arrangement) * $this->val2EnfantOuPlusSepare2 / 100) * -1;
        else
            $reduc = $this->val2EnfantOuPlusSepare2 * -1;
        return number_format($reduc, 3, '.', '');
    }

    public function getReduc2EnfantOuPlusSepare1Age2Vente($arrangement = null)
    {
        if ($this->pour2EnfantOuPlusSepare)
            $reduc = ($this->getSaison()->prixBaseVente($arrangement) * $this->val2EnfantOuPlusSepare2 / 100) * -1;
        else
            $reduc = $this->val2EnfantOuPlusSepare2 * -1;
        if ($this->margepour2EnfantOuPlusSepare)
            $marge = abs($reduc) * $this->marge2EnfantOuPlusSepare / 100;
    ...
    /**
     * Return pourcent of reduction example 0.5
     * @param type $montant
     * @param type $ordre
     * @param type $nbrAdulte
     * @param type $age
     * @return int
     */
    public function getReductionEnfantAchat($montant, $ordre, $nbrAdulte, $age)
    {
        $minAge1 = $this->saison->getHotel()->getFicheTechnique()->getMin1AgeEnfant();
        $minAge2 = $this->saison->getHotel()->getFicheTechnique()->getMin2AgeEnfant();
        $maxAge1 = $this->saison->getHotel()->getFicheTechnique()->getMax1AgeEnfant();
        $maxAge2 = $this->saison->getHotel()->getFicheTechnique()->getMax2AgeEnfant();
        if ($age >= $minAge1 && $age <= $maxAge1)
        {
            if ($ordre == 1 && $nbrAdulte == 1)
            {
                if ($this->pour1Enfant1Adulte)
                    $reduc = ($montant * $this->val1Enfant1Adulte1 / 100) * -1;
                else
                    $reduc = $this->val1Enfant1Adulte1 * -1;
            }
            ...
            {
                if ($this->pour1Enfant2Adulte)
                    $reduc = ($montant * $this->val1Enfant2Adulte1 / 100) * -1;
                else
                    $reduc = $this->val1Enfant2Adulte1 * -1;
            }
            ...
            {
                if ($this->pour1EnfantSepare)
                    $reduc = ($montant * $this->val1EnfantSepare1 / 100) * -1;
                else
                    $reduc = $this->val1EnfantSepare1 * -1;
            }
            if ($ordre > 1 && $nbrAdulte == 1)
            {
                if ($this->pour2Enfant1Adulte)
                    $reduc = ($montant * $this->val2Enfant1Adulte1 / 100) * -1;
                else
                    $reduc = $this->val2Enfant1Adulte1 * -1;
            }
            if ($ordre > 1 && $nbrAdulte == 2)
            {
                if ($this->pour2Enfant2Adulte)
                    $reduc = ($montant * $this->val2Enfant2Adulte1 / 100) * -1;
                else
                    $reduc = $this->val2Enfant2Adulte1 * -1;
            }
            if ($ordre > 1 && $nbrAdulte == 0)
            {
                if ($this->pour2EnfantOuPlusSepare)
                    $reduc = ($montant * $this->val2EnfantOuPlusSepare1 / 100) * -1;
                else
                    $reduc = $this->val2EnfantOuPlusSepare1 * -1;
            }
            return number_format($reduc, 3, '.', '');
        }
        elseif ($age >= $minAge2 && $age <= $maxAge2)
        {
            if ($ordre == 1 && $nbrAdulte == 1)
            {
                if ($this->pour1Enfant1Adulte)
                    $reduc = ($montant * $this->val1Enfant1Adulte2 / 100) * -1;
                else
                    $reduc = $this->val1Enfant1Adulte2 * -1;
            }
            if ($ordre == 1 && $nbrAdulte == 2)
            {
                if ($this->pour1Enfant2Adulte)
                    $reduc = ($montant * $this->val1Enfant2Adulte2 / 100) * -1;
                else
                    $reduc = $this->val1Enfant2Adulte2 * -1;
            }
            if ($ordre == 1 && $nbrAdulte == 0)
            {
                if ($this->pour1EnfantSepare)
                    $reduc = ($montant * $this->val1EnfantSepare2 / 100) * -1;
                else
                    $reduc = $this->val1EnfantSepare2 * -1;
            }
            if ($ordre > 1 && $nbrAdulte == 1)
            {
                if ($this->pour2Enfant1Adulte)
                    $reduc = ($montant * $this->val2Enfant1Adulte2 / 100) * -1;
                else
                    $reduc = $this->val2Enfant1Adulte2 * -1;
            }
            if ($ordre > 1 && $nbrAdulte == 2)
            {
                if ($this->pour2Enfant2Adulte)
                    $reduc = ($montant * $this->val2Enfant2Adulte2 / 100) * -1;
                else
                    $reduc = $this->val2Enfant2Adulte2 * -1;
            }
            if ($ordre > 1 && $nbrAdulte == 0)
            {
                if ($this->pour2EnfantOuPlusSepare)
                    $reduc = ($montant * $this->val2EnfantOuPlusSepare2 / 100) * -1;
                else
                    $reduc = $this->val2EnfantOuPlusSepare2 * -1;
            }
            return number_format($reduc + $marge, 3, '.', '');
     ...
     * @return int
     */
    public function getReductionEnfantVente($montant, $ordre, $nbrAdulte, $age)
    {
        $minAge1 = $this->saison->getHotel()->getFicheTechnique()->getMin1AgeEnfant();
        $minAge2 = $this->saison->getHotel()->getFicheTechnique()->getMin2AgeEnfant();
        $maxAge1 = $this->saison->getHotel()->getFicheTechnique()->getMax1AgeEnfant();
