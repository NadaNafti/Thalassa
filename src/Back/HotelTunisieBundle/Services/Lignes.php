...
class Lignes
{
    ...
    public function ligneSuppSingle(Saison $saison, $arr, $ch, $nbrAdulte, $nbrEnfant, $type)
    {
        ...
        if (($type == 'enfant' && $saison->getSaisonSupp()->getSuppSingleEnfant() && $nbrAdulte == 0 && $nbrEnfant == 1) || ($type == 'adulte' && (($nbrAdulte == 1 && $nbrEnfant == 0) || ( /* $chambre->getType() == 2 && */
                        $nbrEnfant == 1 && $nbrAdulte == 1 && $saison->getHotel()->getFicheTechnique()->getSuppSingle1Adulte1EnfantChDouble()) || ( /* $chambre->getType() == 3 && */
                        $nbrEnfant == 2 && $nbrAdulte == 1 && $saison->getHotel()->getFicheTechnique()->getSuppSingle1Adulte2EnfantChDouble())))
        )
            return array(
                'code'  => 'SUPP-SINGLE',
                'name'  => 'Supp single',
    ...
    public function ligneSuppReduc3Lit(Saison $saison, $arr, $k)
    {
        if ($k == 3)
            return array(
                ...
                'achat' => $saison->getSaisonReduc()->getReduc3LitAchat($arr),
                'vente' => $saison->getSaisonReduc()->getReduc3LitVente($arr),
            );
    }
    ...
    public function ligneSuppReduc4Lit(Saison $saison, $arr, $k)
    {
        if ($k == 4)
            return array(
                ...
                'achat' => $saison->getSaisonReduc()->getReduc4LitAchat($arr),
                'vente' => $saison->getSaisonReduc()->getReduc4LitVente($arr),
            );
    }
                ...
                for ($i = 0; $i < $nbrAdultes; $i++) {
                    for ($j = 0; $j < $nbrJours; $j++)
                        $tab['adultes'][$i]['jours'][$j]['lignes'][] = array(
                            'code'  => 'FRAIS-CHAMBRE',
                            'name'  => 'Frais '  . $fraisChambre->getChambre()->getLibelle() ,
                            'achat' => number_format($fraisChambre->getPrixAchat($nbrAdultes, $nbrEnfants,$arr, 'Adulte', $i+1), 3, '.', ''),
                            'vente' => number_format($fraisChambre->getPrixVente($nbrAdultes, $nbrEnfants,$arr, 'Adulte', $i+1), 3, '.', ''),
                        );
                    for ($j = 0; $j < $nbrJours; $j++) {
                ...
                for ($i = 0; $i < $nbrEnfants; $i++) {
                    for ($j = 0; $j < $nbrJours; $j++)
                        $tab['enfants'][$i]['jours'][$j]['lignes'][] = array(
                            'code'  => 'FRAIS-CHAMBRE',
                            'name'  => 'Frais '  . $fraisChambre->getChambre()->getLibelle(),
                            'achat' => number_format($fraisChambre->getPrixAchat($nbrAdultes, $nbrEnfants,$arr, 'Enfant', $i+1), 3, '.', ''),
                            'vente' => number_format($fraisChambre->getPrixVente($nbrAdultes, $nbrEnfants,$arr, 'Enfant', $i+1), 3, '.', ''),
                        );
                    for ($j = 0; $j < $nbrJours; $j++) {
