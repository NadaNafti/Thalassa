...
namespace Back\TransfertBundle\Controller;

use Back\TransfertBundle\Entity\TransfertReservation;
...
class TransfertController extends Controller
{
    ...
    public function listeAction($page,$etat,$sort,$direction)
    {
        ...
        if($request->isMethod("POST"))
            return $this->redirect($this->generateUrl('back_transfert_reservation_liste',array('etat' => $request->get('etatFiltre'))));
        $reservations = $em->getRepository('BackTransfertBundle:TransfertReservation')->filtre($etat,$sort,$direction);
        $paginator = $this->get('knp_paginator');
        $reservations = $paginator->paginate($reservations,$page,20);
        return $this->render('BackTransfertBundle:transfert:liste.html.twig',array(
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $reservations = $em->getRepository("BackTransfertBundle:TransfertReservation")->findBy(array('etat'=>1),array('id'=>'desc'));
        return $this->render('BackTransfertBundle:transfert:notification.html.twig', array(
            'reservations' => $reservations,
        ));
    }

    public function consulterAction(TransfertReservation $reservation)
    {
        return $this->render('BackTransfertBundle:transfert:consultation.html.twig', array(
    ...
    public function deleteAction(TransfertReservation $reservation)
    {
        ...
        return $this->redirect($this->generateUrl("back_transfert_reservation_liste"));
    }

    public function prendreEnChargeAction(TransfertReservation $reservation)
    {
        ...
        return $this->redirect($this->generateUrl("back_transfert_reservation_consultation", array('id' => $reservation->getId())));
    }

    public function remiseAction(TransfertReservation $reservation)
    {
        $user = $this->get('security.context')->getToken()->getUser();
        $em = $this->getDoctrine()->getManager();
        $session = $this->getRequest()->getSession();
    ...
    public function totalAction(TransfertReservation $reservation)
    {
        $user = $this->get('security.context')->getToken()->getUser();
        $em = $this->getDoctrine()->getManager();
        $session = $this->getRequest()->getSession();
    ...
    public function annulerAction(TransfertReservation $reservation)
    {
        $user = $this->get('security.context')->getToken()->getUser();
        $em = $this->getDoctrine()->getManager();
        $request = $this->getRequest();
    ...
    public function validerAction(TransfertReservation $reservation)
    {
        $user = $this->get('security.context')->getToken()->getUser();
        $em = $this->getDoctrine()->getManager();
        $request = $this->getRequest();
        ...
        if($request->isMethod("POST")){
            ...
            $em->flush();
            return $this->redirect($this->generateUrl("back_transfert_reservation_validation",array('id' => $reservation->getId())));
        }
        return $this->render('BackTransfertBundle:transfert:validation.html.twig',array(
            'reservation' => $reservation,
